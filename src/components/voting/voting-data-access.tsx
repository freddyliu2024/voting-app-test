...
export function useVotingProgram() {
  ...
  const initialize = useMutation({
    ...
    mutationFn: (keypair: Keypair) =>
      program.methods
        .initialize()
        .accounts({ voting: keypair.publicKey })
        .signers([keypair])
        .rpc(),
    onSuccess: (signature) => {
...
export function useVotingProgramAccount({ account }: { account: PublicKey }) {
  ...
  const closeMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.close().accounts({ voting: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accounts.refetch();
    },
  ...
  const decrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.decrement().accounts({ voting: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
  ...
  const incrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.increment().accounts({ voting: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
  ...
  const setMutation = useMutation({
    ...
    mutationFn: (value: number) =>
      program.methods.set(value).accounts({ voting: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
